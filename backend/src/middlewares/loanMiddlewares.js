...
const createMiddleware = (req, res, next) => {
    const errors = validate(req.body, schemas.CREATE_LOAN)
    if (errors.length != 0) return res.status(400).json({status: 400, message: errors})
    next()
}
const updateMiddleware = (req, res, next) => {
    if (Object.keys(req.body).length == 0) return res.status(400).json({status: 400, message: "Nenhum campo foi modificado!"})
    ...
    const errors = validate(req.body, schemas.UPDATE_LOAN)
    if (errors.length != 0) return res.status(400).json({status: 400, message: errors})
    next()
}
